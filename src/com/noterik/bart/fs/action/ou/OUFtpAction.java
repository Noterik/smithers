 ...
 * @project smithers2
 * @access private
 * @version $Id: OUFtpAction.java,v 1.42 2009-06-15 14:00:08 derk Exp $
 *
 */
public class OUFtpAction extends ActionAdapter{

	
	/**
	 * logger
	 */
	private static Logger logger = Logger.getLogger(OUFtpAction.class);
	
	/**
	 * video config uri
	 */
	//private static final String configUri = "/domain/ou/config/video/rawvideo";
	private static final String configUri = "/domain/ou/config/ftpscript";
	...
	public String run() {		
		...
		logger.debug("**************** Starting OU FTP Action ***********");
		...
		logger.debug("properties xml = " + doc.asXML());
		...
		try {
			...
			if(rawUri.indexOf("/rawvideo/1") != -1){
				...
				if( external == null ){
					...
					String extValue = "mms://mms1.openu.local/stream1" + rawUri + "/raw.wmv";
					
					logger.debug("Video 1 - about to set the external uri: " + extValue);
				...
				// there is no status message
					...
					if(stNode.getText().toLowerCase().equals("failed")){
						...
						if(handleFtp(doc, "1", origUri, rawUri)){
							...
							String extValue = "http://telem.openu.ac.il/cgi-bin/telem/tools/open_video/fetch_media.pl?ar=" +  filename + "&cusi=1";
							FSXMLRequestHandler.instance().updateProperty(extUri, "externaluri", extValue, "PUT", false);						
						}
						else{
	...
	private boolean ftpVideo(String fileUri, String rawUri, String stream, String extension, String server, String user, String pass){
		...
		if(!tmpFolder.exists() && !tmpFolder.mkdirs()){
			...
			logger.error("Unable to create the tmp folder");
			return false;
		}
